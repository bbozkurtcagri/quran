...
internal sealed class GetVerseByGlobalNumberQueryHandler(IApplicationDbContext db)
    : IRequestHandler<GetVerseByGlobalNumberQuery, Result<VerseSummaryDto>>
{
    public async Task<Result<VerseSummaryDto>> Handle(
        GetVerseByGlobalNumberQuery request,
        CancellationToken cancellationToken)
    {
        ...
        var verse = await db.Verses
            .AsNoTracking()
            .Where(v => v.GlobalVerseNumber == request.GlobalVerseNumber)
        ...
        if (verse is null)
        {
            return Result.Failure<VerseSummaryDto>(
                Error.NotFound(
                    "verse.not_found",
                    $"Verse with global number {request.GlobalVerseNumber} was not found."));
        }
